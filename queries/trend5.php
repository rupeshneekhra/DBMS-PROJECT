<?php
	...
	$sql = " select * from publication where pub_id in(select distinct made_pub.pub_id from made_pub where made_pub.f_id=? and publication.pub_id=made_pub.pub_id and year(publication.date_)=?) ;";
	
	if($stmt = $conn->prepare($sql)){
		$stmt->bind_param('ss',$id2,$yr);
		$stmt->execute();
		$stmt->store_result();
		if($stmt->num_rows!=0){
